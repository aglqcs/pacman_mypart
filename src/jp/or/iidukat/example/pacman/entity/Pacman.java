...
import jp.or.iidukat.example.pacman.PosInfo;
...
public class Pacman extends PlayfieldActor {
	...
	private static final InitPosition INIT_POS = InitPosition
			.createPlayerInitPosition(39.5f, 15, Direction.LEFT);
	...
	Pacman(Bitmap sourceImage, PacmanGame game, String name) {
		...
		if (this.dir == inputDir.getOpposite()) {
			...
			if (this.dir == Direction.NONE) {
				...
				PathElement p = game.getPathElement(this.tilePos[1],
						this.tilePos[0]);
				if (p != null && p.allow(inputDir)) { // if an available
														// direction is entered
					// determine whether the input of direction is slightly
					// delayed.
					Move mv = this.dir.getMove();
					float[] pastPos = new float[] { this.pos[0], this.pos[1] };
					pastPos[mv.getAxis()] -= mv.getIncrement();
					int stepCount = 0;
					if (pastPos[0] == this.tilePos[0]
							&& pastPos[1] == this.tilePos[1]) {
						stepCount = 1;
					} else {
						pastPos[mv.getAxis()] -= mv.getIncrement();
						if (pastPos[0] == this.tilePos[0]
	...
	private void handlePrecedeInput() {
		...
		if (this.pos[0] >= a[0] && this.pos[0] <= b[0] && this.pos[1] >= a[1]
				&& this.pos[1] <= b[1]) {
	...
	void adjustPosOnEnteringTile(int[] tilePos) {
		if (!game.getPathElement(tilePos[1], tilePos[0]).isPath()) { // try
																		// moving
																		// to
																		// where
																		// is
																		// not a
																		// path
			// correct the position to where Pacman has actually moved to the
			// last
	...
	@Override
	void shortcutCorner() {
		// update the position according to the precede input (see
		// handlePrecedeInput method)
	...
	void handleAnObjectWhenEncountering() {
		...
		if (this.pos[0] == Playfield.FRUIT_POSITION[0]
				&& (this.pos[1] == Playfield.FRUIT_POSITION[1] || this.pos[1] == Playfield.FRUIT_POSITION[1] + 8)) {
			game.eatFruit();
		}
	}
	...
	int[] getImagePos() {
		...
		if (game.getGameplayMode() == GameplayMode.GHOST_DIED) { // eat a ghost.
																	// no image.
			x = 3;
			y = 0;
		} else if (game.getGameplayMode() == GameplayMode.LEVEL_BEING_COMPLETED
				|| game.getGameplayMode() == GameplayMode.LEVEL_COMPLETED) { // complete
																				// a
																				// level.
																				// Pacman
																				// is
			...
			y = 0;
				...
				|| game.getGameplayMode() == GameplayMode.GAME_RESTARTED) { // Immediately
																			// after
																			// game
																			// starts
			x = 2;
			...
			int t = 20 - (int) FloatMath.floor(game.getGameplayModeTime()
					/ game.getTiming()[4] * 21);
			x = t - 1;
			switch (x) {
			case -1:
