...
public class PacmanGame {
	...
	// After a level restart, when a number of dots eaten by the player has
	...
	private static final float[] EVENT_TIME_TABLE = { 0.16f, 0.23f, 1, 1,
			2.23f, 0.3f, 1.9f, 2.23f, 1.9f, 5, 1.9f, 1.18f, 0.3f, 0.5f, 1.9f,
			9, 10, 0.26f };
	...
	private static final LevelConfig[] LEVEL_CONFIGS = {
			...
			new LevelConfig.Builder()
					...
					.ghostModeSwitchTimes(
							new float[] { 7, 20, 7, 20, 5, 20, 5, 1, })
					.penForceTime(4)
					.penLeavingLimits(new float[] { 0, 0, 30, 60, }).build(),
			new LevelConfig.Builder()
					.ghostSpeed(0.85f)
					...
					.fruitScore(300)
					.ghostModeSwitchTimes(
							new float[] { 7, 20, 7, 20, 5, 1033, 1f / 60, 1, })
					.penForceTime(4)
			...
			new LevelConfig.Builder()
					...
					.ghostModeSwitchTimes(
							new float[] { 7, 20, 7, 20, 5, 1033, 1f / 60, 1, })
					.penForceTime(4)
					.penLeavingLimits(new float[] { 0, 0, 0, 0, }).build(),
			new LevelConfig.Builder()
					.ghostSpeed(0.85f)
					.ghostTunnelSpeed(0.45f)
					.playerSpeed(0.9f)
			...
			new LevelConfig.Builder()
					...
					.fruitScore(700)
					.ghostModeSwitchTimes(
							new float[] { 5, 20, 5, 20, 5, 1037, 1f / 60, 1, })
					.penForceTime(3)
	...
	static {
		...
		css.put(Integer.valueOf(1), new Cutscene(new Class<?>[] {
				CutscenePacman.class, CutsceneBlinky.class }, new float[] {
				5.5f, 0.1f, 9 }));
		css.put(Integer.valueOf(2), new Cutscene(
				new Class<?>[] { CutscenePacman.class, CutsceneBlinky.class,
						CutsceneSteak.class }, new float[] { 2.7f, 1, 1.3f, 1,
						2.5f }));
		css.put(Integer.valueOf(3), new Cutscene(new Class<?>[] {
				CutscenePacman.class, CutsceneBlinky.class }, new float[] {
				5.3f, 5.3f }));
	...
	public static enum GameplayMode {
		ORDINARY_PLAYING(0), GHOST_DIED(1), PLAYER_DYING(2), PLAYER_DIED(3), NEWGAME_STARTING(
				4), NEWGAME_STARTED(5), GAME_RESTARTING(6), GAME_RESTARTED(7), GAMEOVER(
				8), LEVEL_BEING_COMPLETED(9), LEVEL_COMPLETED(10), TRANSITION_INTO_NEXT_SCENE(
				11), CUTSCENE(13), KILL_SCREEN(14);
	...
	PacmanGame(Context context, MessagePasser m, String[] names, int i) {
	...
	void handleTouchEnd(MotionEvent e) {
		...
		for (int i = 0; i < 4; i++) {
			if (absDx < 8 && absDy < 8) {
				...
				pacman[i].setRequestedDir(touchDX > 0 ? Direction.RIGHT
						: Direction.LEFT);
			} else if (absDy > 15 && absDx < absDy * 2 / 3) {
				pacman[i].setRequestedDir(touchDY > 0 ? Direction.DOWN
						: Direction.UP);
			}
		}
		cancelTouch();
	...
	private void restartGameplay(boolean newGame) {
		...
		// changeGameplayMode(GameplayMode.LEVEL_COMPLETED); // for Cutscene
		// debug
	}
	...
	private void newLevel(boolean newGame) {
		level++;
		levelConfig = level >= LEVEL_CONFIGS.length ? LEVEL_CONFIGS[LEVEL_CONFIGS.length - 1]
				: LEVEL_CONFIGS[level];
		levelConfig.frightTotalTime = levelConfig.frightTime
	...
	private void switchMainGhostMode(GhostMode ghostMode,
		...
		if (ghostMode == GhostMode.FRIGHTENED && levelConfig.frightTime == 0) {
			for (Ghost ghost : ghosts) {
				ghost.setReverseDirectionsNext(true); // If frightTime is 0, a
														// frightened ghost only
														// reverse its
														// direction.
			}
		} else {
			...
			for (Ghost ghost : ghosts) {
				...
				if (ghost.getMode() != GhostMode.EATEN
						...
						&& ghost.getMode() != GhostMode.ENTERING_PEN
						|| justRestartGame) {

					// If it is not immediately after restart the game
					// (justRestartGmae:false),
					// a ghost reverse its direction
					// when its mode change from other than FRIGHTENED (CHASE or
					// SCATTER) to another mode.
					if (!justRestartGame
							&& ghost.getMode() != GhostMode.FRIGHTENED
							&& ghost.getMode() != ghostMode) {
						ghost.setReverseDirectionsNext(true);
					}

					// If it is not immediately after restart the game
					// and a mode of each ghost is any of EATEN, IN_PEN,
					// LEAVING_PEN, RE_LEAVING_FROM_PEN, or ENTERING_PEN,
					// it is not updated.
	...
	private void figureOutPenLeaving() {
		/*
		 * Ghost pinky = getPinky(); Ghost inky = getInky(); Ghost clyde =
		 * getClyde(); if (alternatePenLeavingScheme) { // By using a number of
		 * dots eaten after a level restart, // manage the timing of the ghosts
		 * leaving from the pen. alternateDotCount++; if (alternateDotCount ==
		 * PEN_LEAVING_FOOD_LIMITS[1]) { pinky.setFreeToLeavePen(true); } else
		 * if (alternateDotCount == PEN_LEAVING_FOOD_LIMITS[2]) {
		 * inky.setFreeToLeavePen(true); } else if (alternateDotCount ==
		 * PEN_LEAVING_FOOD_LIMITS[3]) { if (clyde.getMode() ==
		 * GhostMode.IN_PEN) { alternatePenLeavingScheme = false; } } } else if
		 * (pinky.getMode() == GhostMode.IN_PEN || pinky.getMode() ==
		 * GhostMode.EATEN) { pinky.incrementDotCount(); if (pinky.getDotCount()
		 * >= levelConfig.penLeavingLimits[1]) { pinky.setFreeToLeavePen(true);
		 * } } else if (inky.getMode() == GhostMode.IN_PEN || inky.getMode() ==
		 * GhostMode.EATEN) { inky.incrementDotCount(); if (inky.getDotCount()
		 * >= levelConfig.penLeavingLimits[2]) { inky.setFreeToLeavePen(true); }
		 * } else if (clyde.getMode() == GhostMode.IN_PEN || clyde.getMode() ==
		 * GhostMode.EATEN) { clyde.incrementDotCount(); if (clyde.getDotCount()
		 * >= levelConfig.penLeavingLimits[3]) { clyde.setFreeToLeavePen(true);
		 * } }
		 */
	}
	...
	public void dotEaten(int[] dotPos) {
		...
		playDotEatingSound();
		if (getPathElement(dotPos[1], dotPos[0]).getDot() == Dot.ENERGIZER) { // when
																				// eating
																				// an
																				// energizer
	...
	// an element of this array represents whether it is necessary to move the
	// character in a intervalTime.
	...
	public Boolean[] getSpeedIntervals(float speed) {
		...
		if (!speedIntervals.containsKey(key)) {
			...
			speedIntervals
					.put(key, movabilityTimeTable.toArray(new Boolean[0]));
		}
	...
	private void cutsceneNextSequence() {
		...
		if (cutscene.sequenceTimes.length == cutsceneSequenceId) {
			...
			cutsceneTime = cutscene.sequenceTimes[cutsceneSequenceId]
					* DEFAULT_FPS;
					...
					for (Ghost ghost : ghosts) {
						if (ghost.getMode() == GhostMode.FRIGHTENED
								|| (ghost.getMode() == GhostMode.IN_PEN || ghost
										.getMode() == GhostMode.RE_LEAVING_FROM_PEN)
		...
		if (frightModeTime != 0) {
			...
			if (ghostModeTime <= 0) {
				...
				if (ghostModeSwitchPos < levelConfig.ghostModeSwitchTimes.length) {
					ghostModeTime = levelConfig.ghostModeSwitchTimes[ghostModeSwitchPos]
							* DEFAULT_FPS;
	...
	private void handleForcePenLeaveTimer() {
		if (forcePenLeaveTime != 0) {
			...
			if (forcePenLeaveTime <= 0) {
				Ghost[] ghosts = getGhosts();
				/*
				 * for (int i = 1; i <= 3; i++) { if (ghosts[i].getMode() ==
				 * GhostMode.IN_PEN) { ghosts[i].setFreeToLeavePen(true); break;
				 * } }
				 */
				resetForcePenLeaveTime();
			}
	...
	void tick() {
		...
		lastTimeDelta += now - lastTime - tickInterval; // total processing
														// delay
		if (lastTimeDelta > 100) {
			lastTimeDelta = 100;
		}
		if (canDecreaseFps && lastTimeDelta > 50) {
			// If the fps can be reduced, count the number of process which
			// latency is over 50 ms.
			lastTimeSlownessCount++;
			if (lastTimeSlownessCount == 20) {
				decreaseFps(); // reduce the fps when the number of process,
								// which latency is over 50 ms, becomes 20.
			}
		}
		...
		if (lastTimeDelta > tickInterval) {
			...
			latencyMultiplyer = (int) FloatMath.floor(lastTimeDelta
					/ tickInterval);
	...
	private void playDotEatingSound() {
		if (pacManSound) {
			if (gameplayMode == GameplayMode.ORDINARY_PLAYING) {
				String track = dotEatingSoundPart == 1 ? "eating_dot_1"
						: "eating_dot_2";
				playSound(track, 1 + dotEatingChannel, true);
				dotEatingChannel = (dotEatingChannel + 1) % 2; // switch between
																// 0 and 1
				dotEatingSoundPart = 3 - dotEatingSoundPart; // switch between 1
																// and 2
			}
		}
	}
	...
	public void playAmbientSound() {
		if (pacManSound) {
			...
			if (gameplayMode == GameplayMode.ORDINARY_PLAYING
				...
				ambient = ghostEyesCount != 0 ? "ambient_eyes"
						: mainGhostMode == GhostMode.FRIGHTENED ? "ambient_fright"
								: playfieldEl.getDotsEaten() > 241 ? "ambient_4"
										: playfieldEl.getDotsEaten() > 207 ? "ambient_3"
												: playfieldEl.getDotsEaten() > 138 ? "ambient_2"
														: "ambient_1";
			}
	...
	private void initializeTickTimer() {
		...
		for (int i = 0; i < EVENT_TIME_TABLE.length; i++) {
			float sec = !pacManSound && (i == 7 || i == 8) ? 1
					: EVENT_TIME_TABLE[i];
